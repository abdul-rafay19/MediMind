...
async function runTriage() {
  if (!token) {
    openModal('login');
    toast('Please sign in to analyze symptoms', 'error');
    return;
  }
  ...
  try {
    if (token && currentSessionId) {
      ...
      const res = await fetch(API + '/triage/followup', {
        ...
        body: JSON.stringify({ session_id: String(currentSessionId), question: q, language: 'en' }),
      });
...
async function downloadReport() {
  ...
  try {
    const res = await fetch(API + '/reports/generate', {
      ...
      body: JSON.stringify({ session_id: String(currentSessionId), patient_name: currentUser.full_name }),
    });
